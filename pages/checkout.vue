...
const products = computed(() => {
  ...
  if (productId) {
    const filteredProduct = allProducts.find(p => p.id === productId);
...
const validateFormCompletion = async () => {
  ...
  try {
    ...
    // Check if any quiz completion flag exists (since we're not using quizId anymore)
    // Look for any completed quiz in localStorage
    let isAnyQuizCompleted = false;
    ...
    if (!isAnyQuizCompleted) {
      // No quiz completed, redirect to consultation with productId
      const consultationUrl = productId ? `/consultation?productId=${productId}` : '/consultation';
      await navigateTo(consultationUrl);
      return;
    }

    // Quiz is completed, load the completed form data
